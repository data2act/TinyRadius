...
import com.data2act.tinyradius.attribute.VendorSpecificAttribute;
import com.data2act.tinyradius.dictionary.AttributeType;
import com.data2act.tinyradius.dictionary.DefaultDictionary;
...
public class RadiusPacket {
	...
	public void encodeRequestPacket(OutputStream out, String sharedSecret) throws IOException {
		encodeRequestPacket(out, sharedSecret, false);
	}
	 ...
	 *            shared secret to be used to encode this packet
	 * @param signPacket
	 *            if true, add the Message-Authenticator attribute
	 * @exception IOException
	 *                communication error
	 */
	public void encodeRequestPacket(OutputStream out, String sharedSecret, boolean signPacket) throws IOException {
		encodePacket(out, sharedSecret, null, signPacket);
	}
	...
	public void encodeResponsePacket(OutputStream out, String sharedSecret, RadiusPacket request) throws IOException {
		...
		encodeResponsePacket(out, sharedSecret, request, false);
	}

	/**
	 * Encodes this Radius response packet and sends it to the specified output
	 * stream. and sends it to the specified output stream.
	 * 
	 * @param out
	 *            output stream to use
	 * @param sharedSecret
	 ...
	 *                communication error
	 */
	public void encodeResponsePacket(OutputStream out, String sharedSecret, RadiusPacket request, boolean signPacket)
			throws IOException {
		if (request == null)
			throw new NullPointerException("request cannot be null");
		encodePacket(out, sharedSecret, request, signPacket);
	}
	...
	protected void encodePacket(OutputStream out, String sharedSecret, RadiusPacket request) throws IOException {
		encodePacket(out, sharedSecret, request, false);
	}
	 ...
	 *            response packet, null if this packet is a request packet
	 * @param signPacket
	 *            if true, add the Message-Authenticator attribute to this
	 *            packet; before encoding it if this packet is a response packet
	 *            and after encoding it if it is a request packet
	 * @exception IOException
	 *                communication error
	 * @exception RuntimeException
	 *                if required packet data has not been set
	 */
	protected void encodePacket(OutputStream out, String sharedSecret, RadiusPacket request, boolean signPacket)
			throws IOException {
		// check shared secret
		...
		if (signPacket) {
			signPacket(request == null ? this : request, sharedSecret);
		}
	 ...
	 *
	 * @param request
	 *            Radius request packet associated with this packet, possibly
	 *            {@code this}
	 * @param sharedSecret
	 *            shared secret to be used to encode this packet
	 */
	private void signPacket(RadiusPacket request, String sharedSecret) {
		final int messageAuthenticatorCode = dictionary.getAttributeTypeByName("Message-Authenticator").getTypeCode();
		RadiusAttribute att = getAttribute(messageAuthenticatorCode);
		if (att == null) {
			att = new RadiusAttribute(messageAuthenticatorCode, new byte[16]);
			addAttribute(att);
		}
		// compose a temporary response packet to compute the msg authenticator
		ByteBuffer buf = ByteBuffer.allocate(MAX_PACKET_LENGTH);
		buf.put((byte) getPacketType());
		...
		// copy attributes
		for (RadiusAttribute attr : (List<RadiusAttribute>) getAttributes()) {
		...
		try {
			...
			att.setAttributeData(signature);
		} catch (Exception e) {
	...
	/**
	 * Returns whether the Message-Authenticator attribute of this packet
	 * contains the correct value per
	 * http://tools.ietf.org/html/rfc2869#section-5.14 if
	 * {@code getPacketType() == ACCESS_REQUEST}; otherwise returns false.
	 *
	 * @see AccessRequest#hasValidMessageAuthenticator(String)
	 */
	public boolean hasValidMessageAuthenticator(String sharedSecret) {
		return (getPacketType() == ACCESS_REQUEST) && hasValidMessageAuthenticator(sharedSecret, this);
	}

	/**
	 * Returns whether the Message-Authenticator attribute of this response
	 * packet contains the correct value per
	 * http://tools.ietf.org/html/rfc2869#section-5.14.
	 * <p>
	 * If this packet is an {@link AccessRequest}, this method may modify it
	 * under incorrect usage; see
	 * {@link AccessRequest#hasValidMessageAuthenticator}. Otherwise, this
	 * packet is not modified.
	 *
	 * @param sharedSecret
	 *            shared secret to be used to validate the Message-Authenticator
	 *            of this packet
	 * @param request
	 *            Radius request packet associated with this packet, possibly
	 *            {@code this}
	 * @returns true if the Message-Authenticator field is correctly set within
	 *          this packet; false otherwise
	 */
	public boolean hasValidMessageAuthenticator(String sharedSecret, RadiusPacket request) {
		final int messageAuthenticatorCode = dictionary.getAttributeTypeByName("Message-Authenticator").getTypeCode();
		RadiusAttribute existingSignature = getAttribute(messageAuthenticatorCode);
		if (existingSignature == null)
			return false;

		try {
			encodeRequestAttributes(sharedSecret); // does nothing if this is
													// not an AccessRequest

			ByteBuffer attributesBuf = ByteBuffer.allocate(MAX_PACKET_LENGTH - RADIUS_HEADER_LENGTH);
			for (RadiusAttribute attr : (List<RadiusAttribute>) getAttributes()) {
				if (attr.getAttributeType() == messageAuthenticatorCode) {
					attributesBuf.put(new RadiusAttribute(messageAuthenticatorCode, new byte[16]).writeAttribute());
				} else {
					attributesBuf.put(attr.writeAttribute());
				}
			}

			decodeRequestAttributes(sharedSecret); // does nothing if this is
													// not an AccessRequest

			ByteBuffer buf = ByteBuffer.allocate(MAX_PACKET_LENGTH);
			buf.put((byte) getPacketType());
			buf.put((byte) getPacketIdentifier());
			buf.putShort((short) (RADIUS_HEADER_LENGTH + attributesBuf.position()));
			buf.put(request.getAuthenticator());
			attributesBuf.flip();
			buf.put(attributesBuf);
			buf.flip();
			...
			byte[] signature = new byte[16];
			System.arraycopy(mac.doFinal(), 0, signature, 0, signature.length);
			return Arrays.equals(existingSignature.getAttributeData(), signature);
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
	}
